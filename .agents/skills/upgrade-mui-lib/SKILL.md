...
**Do NOT upgrade to TypeScript 6** — API Extractor (used by `vite-plugin-dts` for `rollupTypes`) bundles TS 5.7.x and produces an empty `export { }` declaration file with TS 6.
...
- `interface` → `type` (`@typescript-eslint/consistent-type-definitions`) — **CRITICAL: revert on ALL exported interfaces** (e.g., `MuiColorInputProps`, `MuiTelInputProps`). Consumers rely on declaration merging. The autofix silently converts `interface Foo extends Bar {}` to `type Foo = {} & Bar` which is a **breaking change**. After running `--fix`, always run `git diff -- src/index.types.ts` and revert any `interface` → `type` conversion on public types. Add `// eslint-disable-next-line @typescript-eslint/consistent-type-definitions -- interface required for consumer declaration merging` above each reverted interface.
